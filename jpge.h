...
private:
    ...
    int16 *m_coefficient_array;
    int32 *m_quantization_tables[2];
    uint *m_huff_codes[4];
    uint8 *m_huff_code_sizes[4];
    uint8 *m_huff_bits[4];
    uint8 *m_huff_val[4];
    int m_last_dc_val[3];
    uint8 *m_out_buf;
    ...
    void emit_sos();
    void emit_markers();
    bool compute_huffman_table(uint **codes, uint8 **code_sizes, uint8 *bits, uint8 *val);
    ...
    void load_coefficients(int component_num);
    void flush_output_buffer();
    void put_bits(uint bits, uint8 len);
